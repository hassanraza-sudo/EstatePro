    ...
    <div className="h-screen flex overflow-hidden bg-gradient-to-tr from-gray-500 to-gray-500">
      ...
      <div className="hidden md:flex md:w-64 md:flex-col md:border-r md:border-gray-500 bg-slate-600 shadow-sm">
        <div className="flex items-center px-4 py-5 border-b">
          ...
          <span className="ml-2 text-xl font-serif font-semibold text-gray-100">
            EstatePro
          </span>
        </div>
        <div className="flex items-center space-x-4 px-4 py-4 border-b">
          <div className="h-12 w-12 bg-zinc-300 rounded-full text-gray-700 flex items-center justify-center font-sans font-bold">
            {currentUser?.name?.charAt(0) || "U"}
          </div>
          <div>
            <div className="text-lg font-serif font-medium text-gray-200">
        ...
        <nav className="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
          {sidebarLinks.map((link) => (
            ...
            >
              ...
              <span className="ml-3">{link.name}</span>
              {location.pathname === link.path && (
                <ChevronRight className="ml-auto h-4 w-4 text-sky-50" />
              )}
            </Link>
          ))}
        </nav>
        <div className="px-4 py-4 border-t">
          ...
          >
            <BiLogOutCircle className="h-5 w-5 ml-1 mr-2" />
            Logout
          </button>
        </div>
      </div>
